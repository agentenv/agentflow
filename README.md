...
Doctor also warns when a local Codex or Claude node would inherit an ambient `OPENAI_BASE_URL` or `ANTHROPIC_BASE_URL` from the current shell because the pipeline does not pin that routing explicitly. That keeps shell-specific relay settings visible in preflight output instead of silently affecting the run.
