...
When those custom Kimi-backed preflights succeed, `doctor` and `check-local` also print the matching per-node local readiness probes such as `claude --version`, `codex --version`, and Codex auth readiness, so you can see that AgentFlow actually exercised the prepared shell bootstrap before the run starts.
