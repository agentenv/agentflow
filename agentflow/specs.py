...
def resolve_provider(value: str | ProviderConfig | None, agent: str | AgentKind) -> ProviderConfig | None:
    ...
    if alias == "minimax":
        ...
        # it cannot back Codex nodes. CN/custom endpoints can be supplied via a full
        # ProviderConfig instead of the alias.
        if resolved_agent == AgentKind.CLAUDE:
            return ProviderConfig(
                name="minimax",
        ...
        raise ValueError(
            "provider 'minimax' is only supported for `pi` and `claude` nodes; MiniMax's "
